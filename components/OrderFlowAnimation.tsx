...
// Phone screen components — kept dark (simulates a phone display on white background)
...
function MenuScreen() {
  const items = [
    { name: "Tryffelrisotto", price: "199 kr", tag: "Populär" },
    { name: "Burratrasallad", price: "129 kr", tag: null },
    { name: "Lammkotletter", price: "299 kr", tag: "Nytt" },
  ]
  return (
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 px-3 py-2 space-y-2">
        <div className="flex items-center justify-between bg-stone-800/60 rounded-xl px-3 py-2.5">
          ...
          <div className="flex items-center gap-2">
            <span className="text-amber-400 text-xs font-semibold">199 kr</span>
        ...
        <div className="flex items-center justify-between bg-stone-800/60 rounded-xl px-3 py-2.5">
          ...
          <div className="flex items-center gap-2">
            <span className="text-amber-400 text-xs font-semibold">129 kr</span>
            <div className="flex items-center gap-1">
              <div className="w-5 h-5 rounded-full bg-stone-700 flex items-center justify-center text-stone-300 text-xs">−</div>
              <span className="text-stone-100 text-xs w-3 text-center">1</span>
              <div className="w-5 h-5 rounded-full bg-amber-500/30 flex items-center justify-center text-amber-400 text-xs">+</div>
      ...
      <div className="px-3 pb-3">
        <div className="flex justify-between items-center mb-2 px-1">
          ...
          <span className="text-stone-100 text-xs font-bold">328 kr</span>
        </div>
    ...
    <div className="flex flex-col items-center justify-center h-full gap-4 px-4">
      ...
      <div className="w-full bg-stone-800/60 rounded-xl p-3 space-y-1.5">
        <div className="flex justify-between">
          ...
          <span className="text-amber-400 text-xs">199 kr</span>
        </div>
        <div className="flex justify-between">
          <span className="text-stone-400 text-xs">Burratrasallad</span>
          <span className="text-amber-400 text-xs">129 kr</span>
        </div>
    ...
    <section id="how-it-works" className="bg-gray-50 py-24">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex flex-col lg:flex-row gap-10 items-center lg:items-start">
                ...
                >
                  <div
                    className={`shrink-0 w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${
                      isActive ? "bg-amber-500/20" : "bg-stone-100"
                    }`}
                  >
                    <Icon className={`h-4 w-4 ${isActive ? "text-amber-500" : "text-stone-500"}`} />
                  </div>
                  <div className="min-w-0">
                    <div className={`text-sm font-semibold mb-0.5 ${isActive ? "text-amber-500" : "text-stone-500"}`}>
                      {step.title}
                    </div>
                    {isActive && (
                      <div className="text-stone-600 text-xs leading-relaxed">{step.desc}</div>
                    )}
                    {isActive && (
          ...
          {/* Phone mockup — kept dark, simulates a phone screen */}
          <div className="flex-1 flex justify-center lg:justify-end">
            <div className="relative w-64 sm:w-72">
              ...
              {/* Step badge */}
              <div className="absolute -right-4 sm:-right-6 top-1/2 -translate-y-1/2 bg-white border border-stone-200 rounded-xl px-3 py-2 shadow-md flex items-center gap-2">
                <ActiveIcon className="h-3.5 w-3.5 text-amber-500" />
                <span className="text-stone-800 text-xs font-medium">{activeStep + 1} / {steps.length}</span>
              </div>
            </div>
          </div>
        </div>
